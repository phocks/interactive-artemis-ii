<script lang="ts">
  ...
  import rocketUrl from "../assets/rocket.gltf?url";
  const baseLocal = "http://localhost:5173";

  const localRocketUrl = `${baseLocal}${rocketUrl}`;
  const remoteRocketUrl = `${rocketUrl}`;

  const rocketModelUrl = import.meta.env.DEV ? localRocketUrl : remoteRocketUrl;
